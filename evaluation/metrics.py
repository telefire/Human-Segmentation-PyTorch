...
def miou(logits, targets, eps=1e-6):
	...
	print(outputs.size())
	print(torch.tensor(1.0).cuda().size())
	outputs = torch.zeros_like(logits).scatter_(dim=1, index=outputs, src=torch.tensor(1.0).cuda()).type(torch.int8)
	targets = torch.zeros_like(logits).scatter_(dim=1, index=targets, src=torch.tensor(1.0).cuda()).type(torch.int8)
...
def iou_with_sigmoid(sigmoid, targets, eps=1e-6):
	...
	#print(outputs.size())
	outputs = outputs.resize_(4,1,320,320)
